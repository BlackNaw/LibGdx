...
public class GifDecoder {
	/**
	 * Method to load a GIF to the project
	 * 
	 * @param name
	 *            Name of GIF file with extension
	 * @return Animation
	 * @return
	 */
	public static Animation<?> loadGIF(String name) {
		return gdec.loadGIFAnimation(Animation.PlayMode.LOOP, Gdx.files.internal(name).read());
	}
	...
	// variable:
//	Animation gif= GifDecoder.loadGIF("name.gif");
//	float frameCounter+= Gdx.graphics.getDeltaTime();

	// command:
//	batch.draw((TextureRegion)gif.getKeyFrame(frameCounter,true),0,0);

	// Or the next line if you need rotation, scale etc...

	// command:
	//batch.draw((TextureRegion)gif.getKeyFrame(frameCounter,true),0,0,((TextureRegion)gif.getKeyFrame(1f)).getRegionWidth()/2,((TextureRegion)gif.getKeyFrame(1f)).getRegionHeight()/2,
	// Gdx.graphics.getWidth(),Gdx.graphics.getHeight(),1f,1f,-90f);
	...
	private static class DixieMap extends Pixmap {
		...
		private void setPixels(int[] pixels, int offset, int stride, int x, int y, int width, int height) {
	 ...
	 */
	private int read(InputStream is, int contentLength) {
	...
	protected void setPixels(int frameIndex) {
		...
		// currentImage.getPixels(copyScratch, 0, width, 0, 0, width, height);
		//
		// //previousImage.setPixels(copyScratch, 0, width, 0, 0, width,
		// height);
		// previousImage = new DixieMap(copyScratch, width, height,
		// Format.RGBA8888);
	...
	protected void readBitmap() {
		...
		decodeBitmapData(null, mainPixels); // false decode pixel data to
											// advance buffer
		skip();
		if (err()) {
			return;
		...
		for (h = 0; h < hzones; h++) {
			for (v = 0; v < vzones; v++) {
				...
				if (frameCountCurrent < nrFrames) {
					...
					// target.fillRectangle(xPos, yPos, frame.getWidth(),
					// pixelSize);
					// target.fillRectangle(xPos + frame.getWidth()-pixelSize,
					// yPos, pixelSize, frame.getHeight());
					// target.fillRectangle(xPos, yPos +
					// frame.getHeight()-pixelSize, frame.getWidth(),
					// pixelSize);
					// target.fillRectangle(xPos, yPos,pixelSize,
					// frame.getHeight());
				}
			}
		}
	...
	private static GifDecoder gdec = new GifDecoder();

	private Animation loadGIFAnimation(PlayMode playType, InputStream is) {
		byte[] bytes;
		try {
			bytes = readFully(is);
			gdec.read(bytes);
