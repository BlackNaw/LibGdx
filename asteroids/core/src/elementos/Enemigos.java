...
public  abstract class Enemigos extends Elemento
	...
	public Enemigos(Texture imagen) {
	...
	public void cambiar() {
		...
		this.posicion.y = Gdx.graphics.getBackBufferHeight() + sorteo(Gdx.graphics.getBackBufferHeight());
	...
	public boolean actualizar(Rectangulo cuerpo) {
		if (enLimite()) {
			...
			if (this.posicion.y <= Gdx.graphics.getHeight()
					&& this.posicion.y >= (Gdx.graphics.getHeight() * (50 - this.velocidad * 3) / 100)) {
				this.viva = true;
				disparar(true);
				disparo.mover(-1);
			} else {
	...
	@Override
	public abstract void explotar(SpriteBatch batch);
}
